# This script processes the scRNAseq data from white matter from Seeker et al. 2023
...
# usage: python qc_scrna_11_Seeker_2023.py id age tissue

def main():
# setting up #####
    id = sys.argv[1]
    age = sys.argv[2] #input: Young or Old
    tissue = sys.argv[3] #input: BA4, CB, or CSC
    base_dir = "Preprocessing_scRNA/data" #change here if necessary
    h5ad_path = "80a4eb91-dab2-40dc-9856-cb6de4eb954e.h5ad" #change here per data
    gene_names_org = "symbol" #change here per data

    # create dev_age dict
    tissue_dict = {"BA4": "Brodmann (1909) area 4",
                    "CB": "white matter of cerebellum",
                    "CSC": "cervical spinal cord white matter"}
    ...
    # separate for tissue and agegroup
    print("Subsetting the adata for tissue ", tissue, " and for age ", age)

    cell_id = metadata[(metadata["tissue"] == tissue_dict[tissue]) & (metadata["AgeGroup"] == age)]["cell_id"]
    ...
    adata_subset.obs.rename(columns={"broad_cell_type": "cell_type_level_1"}, inplace = True)
    adata_subset.obs.rename(columns={"cell_type": "cell_type_level_2"}, inplace = True)
    adata_subset.obs.rename(columns={"author_cell_type": "cell_type_level_3"}, inplace = True)
    ...
    level3_cts = adata_subset.obs["cell_type_level_3"].dropna().unique()
