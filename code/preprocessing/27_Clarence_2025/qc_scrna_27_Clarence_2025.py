# This script processes the scRNAseq data from Roussos
...
import sys
# usage: python qc_scrna_27_Clarence_2025.py {id} CaudateNucleus PostnatalEarly
def main():
# setting up #####
    id = sys.argv[1]
    tissue = sys.argv[2]
    ...
    tissue_dict = {
        ...
        "HippocampalFormation": "hippocampal formation",
        "DorsolateralPrefrontalCortex": "dorsolateral prefrontal cortex",
        "AnteriorCingulateCortex": "anterior cingulate cortex"
    }

    base_dir = "Preprocessing_scRNA/data" #change here if necessary
    h5ad_path = "ecd5537e-9561-43f2-9bb0-9d0b911084fe.h5ad" #change here per data
    ...
    if dev_age == "PostnatalEarly":
        cell_id = metadata[(metadata["tissue"] == tissue_dict[tissue]) & (metadata["development_stage"] == 'infant stage') | (metadata["development_stage"] == '4-year-old stage') | (metadata["development_stage"] == '6-year-old stage')]["cell_id"] #change here
    elif dev_age == "PostnatalLate":
        cell_id = metadata[(metadata["tissue"] == tissue_dict[tissue]) & (metadata["development_stage"] == '14-year-old stage') | (metadata["development_stage"] == '20-year-old stage') | (metadata["development_stage"] == '39-year-old stage') | (metadata["development_stage"] == '61-year-old stage') | (metadata["development_stage"] == '62-year-old stage')]["cell_id"] 
