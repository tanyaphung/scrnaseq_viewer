# This script processes the scRNAseq data from Tadross 2025
...
import sys
# usage: python qc_scrna_28_Tadross_2025.py 514_Tadross_Human_2025_Hypothalamus_Tadross Tadross
def main():
# setting up #####
    id = sys.argv[1]
    dataset = sys.argv[2]
    ...
    h5ad_path = "51ed6415-8523-4bbf-9db1-c690945fa5c4.h5ad" #change here per data
    ...
    if dataset == "Tadross":
        cell_id = metadata[(metadata["is_primary_data"] == True)]["index"] 
    else: 
        cell_id = metadata[(metadata["is_primary_data"] == False)]["index"] 
    adata.obs.set_index('index', inplace=True)
    adata_subset = adata[cell_id]
    print("Subsetting adata for dataset is finished.", file=log)
    ...
    adata_subset.obs.rename(columns={"C2_named": "cell_type_level_2"}, inplace = True)
