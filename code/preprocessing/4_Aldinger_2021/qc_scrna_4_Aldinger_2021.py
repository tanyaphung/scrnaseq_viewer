# This script processes the scRNAseq data from prenatal cerebellum from Aldinger et al. 2021
...
# usage: python qc_scrna_4_Aldinger_2021.py 264_Aldinger2021_PrenatalCerebellum_Human_2021_9wpc 9wpc
...
# setting up #####
    ...
    h5ad_path = "14c2c322-e424-47b3-a7af-dd19df10b644.h5ad" #change here per data
    gene_names_org = "symbol" #change here per data

    # create dev_age dict
    dev_age_dict = {"9wpc": "9th week post-fertilization human stage",
                    "10wpc": "10th week post-fertilization human stage",
                    "11wpc": "11th week post-fertilization human stage",
                    "12wpc": "12th week post-fertilization human stage",
                    "14wpc": "14th week post-fertilization human stage",
                    ...
                    "21wpc": "21st week post-fertilization human stage"}
    ...
    log_dev_age = open(os.path.join(base_dir, id_age, "log.txt"), "w")
    clean_adata_dev_age_fp = os.path.join(base_dir, id_age, id_age + ".h5ad")

    # format table 2
    table2_dev_age = open(os.path.join(base_dir, id_age, "table2.csv"), "w")
    header = ["Description", "Number of cells", "Number of genes"]
    print(",".join(header), file=table2_dev_age)

    # format table 3
    table3_dev_age = open(os.path.join(base_dir, id_age, "table3.csv"), "w")
    
    cell_id_age = metadata[(metadata["development_stage"] == dev_age_dict[dev_age])]["cell_id"]
    adata.obs.set_index('index', inplace=True)
    
    adata_dev_age_subset = adata[cell_id_age]
    ...
    adata_dev_age_subset.obs.rename(columns={"fig_cell_type": "cell_type_level_2"}, inplace = True)
