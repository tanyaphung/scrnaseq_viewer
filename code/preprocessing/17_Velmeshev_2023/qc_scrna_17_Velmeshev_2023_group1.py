# This script processes the scRNAseq data from prenatal and postnatal from Velmeshev et al. 2023 Science group_1
...
def main():
    ...
    # --------------------------------------
    # processing for group_1 (2nd trimester)
    # --------------------------------------
    print("Subsetting the adata for sequencing group_1 2nd trimester")
    id_group1 = "228_Velmeshev2023_PrePostNatal_Human_2023_group1"
    ...
    cell_id_group1 = metadata[(metadata["development_stage"] == "fifth LMP month human stage") | (metadata["development_stage"] == "sixth LMP month human stage") | (metadata["development_stage"] == "fourth LMP month human stage")]["cell_id"]
    adata.obs.set_index('index', inplace=True)
    
    adata_group1_subset = adata[cell_id_group1]
    # adata_group1_subset.X = adata_group1_subset.raw.X.copy() #copy the raw layer to X layer #not sure why this command is throwing an integer error
    ...
    print(adata_group1_subset.X.A[1:10,1:10],file=log_group1)
    print(adata_group1_subset.raw.X.A[1:10,1:10],file=log_group1)
