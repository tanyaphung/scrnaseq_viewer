# This script processes the scRNAseq data from adult human M1 from Bakken et al. 2021
...
# usage: python qc_scrna_6_Bakken_2021.py {id} {/path/to/h5ad/}

def main():
# setting up #####
    id = sys.argv[1]
    base_dir = "Preprocessing_scRNA/data" #change here if necessary
    h5ad_path = sys.argv[2] #change here per data
    ...
    # since the adata is for all of the organisms together, we will need to separate them
    print("Subsetting the adata for human")
    ...
    cell_id = metadata[(metadata["organism"] == "Homo sapiens")]["cell_id"]
    adata.obs.set_index('index', inplace=True)
    
    adata_subset = adata[cell_id]

    print("Subsetting adata for human is finished.", file=log)
    ...
    adata_subset.obs.rename(columns={"BICCN_subclass_label": "cell_type_level_2"}, inplace = True)
    adata_subset.obs.rename(columns={"BICCN_cluster_label": "cell_type_level_3"}, inplace = True)

    # modify the adata.var so that we have the row index of adata.var is symbol and ensemble column is named ensembl column here
