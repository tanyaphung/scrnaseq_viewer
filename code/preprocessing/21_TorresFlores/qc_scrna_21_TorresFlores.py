...
# setting up #####
    ...
    # since the adata is for both disease and normal, we need to extract the normal only
    print("Subsetting the adata for normal tissue")
    ...
    adata_normal_subset = adata[cell_id]

    print("Subsetting adata for group is finished.", file=log)
    print("Viewing the adata observations.", file=log)
    print(adata_normal_subset.obs, file=log)

    print("Viewing the adata variables.", file=log)
    print(adata_normal_subset.var, file=log)

    print("Viewing the adata matrix - are these integer counts?",file=log)
    print(adata_normal_subset.X.A[1:25,1:25],file=log)

    adata_nrow = adata_normal_subset.shape[0]
    adata_ncol = adata_normal_subset.shape[1]
    print(f"After subsetting, adata has {adata_nrow} cells and {adata_ncol} genes.", file=log)
    original = ["Original", str(adata_normal_subset.n_obs), str(adata_normal_subset.n_vars)]
    print(",".join(original), file=table2)

    # mitochondrial genes
    adata_normal_subset.var['mt'] = adata_normal_subset.var["feature_name"].str.startswith("MT-")
    # count the number of genes that are mitochondrial genes
    n_mt_genes = np.count_nonzero(adata_normal_subset.var['mt'])
    print(f"adata has {n_mt_genes} mitochondrial genes.", file=log)

    # calculate qc metrics
    sc.pp.calculate_qc_metrics(adata_normal_subset, qc_vars=['mt'], percent_top=None, log1p=False, inplace=True)

    # first filtering: accept cells with at least 200 detected genes and genes need to be expressed in at least 3 cells
    sc.pp.filter_cells(adata_normal_subset, min_genes=200)
    sc.pp.filter_genes(adata_normal_subset, min_cells=3)

    print(f"After first filtering (see documentation for definition), adata has {adata_normal_subset.n_obs} cells and {adata_normal_subset.n_vars} genes.", file=log)
    # save to table 2
    first_filter = ["Filter#1", str(adata_normal_subset.n_obs), str(adata_normal_subset.n_vars)]
    print(",".join(first_filter), file=table2)

    # second filtering based on mt percentage
    adata_normal_subset = adata_normal_subset[adata_normal_subset.obs['pct_counts_mt'] < 10, :]
    print(f"After second filtering (see documentation for definition), adata has {adata_normal_subset.n_obs} cells and {adata_normal_subset.n_vars} genes.", file=log)
    # save to table 2
    second_filter = ["Filter#2", str(adata_normal_subset.n_obs), str(adata_normal_subset.n_vars)]
    print(",".join(second_filter), file=table2)

    # rename cell type columns
    adata_normal_subset.obs.rename(columns={"customclassif": "cell_type_level_1"}, inplace = True)

    # modify the adata.var so that we have the row index of adata.var is symbol and ensemble column is named ensembl
    adata_normal_subset.var.rename(columns={"ensg_id": "ensg_id_orig"}, inplace = True) #the row name ensg_id exists both as row name and column name so I'm renaming the column here
    ...
    adata_normal_subset.var.set_index('symbol', inplace=True) # set the index to gene symbol
    # save to table 2
    ensembl_convertable = ["Ngenes with ensembl", str(adata_normal_subset.n_obs), str(ngenes_after_conversion)]
    print(",".join(ensembl_convertable), file=table2)

    # save
    adata_normal_subset.write_h5ad(filename=clean_adata_fp)

    # print out the saved adata
    print(adata_normal_subset, file=log)
    print(adata_normal_subset.var, file=log) 
    print(adata_normal_subset.obs, file=log)       

    # tabulate the number of cells per cell type
    # define the available cell types in the dataset
    level1_cts = adata_normal_subset.obs["cell_type_level_1"].dropna().unique()
    for ct in level1_cts:
        ct_data = adata_normal_subset[adata_normal_subset.obs["cell_type_level_1"] == ct, :].X
