# This script processes the scRNAseq data from white matter from Jakel et al. 2019
...
# usage: python qc_scrna_2_Jakel_2019.py 5_Jakel_WhiteMatter_Human_2019

def main():
    # setting up #####
    id = sys.argv[1]
    base_dir = "Preprocessing_scRNA/data" #change here if necessary
    h5ad_path = os.path.join(base_dir, id, "57cd8c07-9434-4b4a-923f-8ae275e20b8e.h5ad") #change here per data
    ...
    metadata = metadata.rename(columns = {'Detected': 'cell_id'})
    ...
    adata.obs.set_index('Detected', inplace=True)
    ...
    adata_subset.obs.rename(columns={"author_cell_types": "cell_type_level_2"}, inplace = True)
