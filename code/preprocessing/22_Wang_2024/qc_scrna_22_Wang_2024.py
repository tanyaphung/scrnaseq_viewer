# This script processes the scRNAseq data from neocortex from Wang et al. 2024
...
# usage: python qc_scrna_22_Wang_2024.py 429_Wang2024_Human_2024_FirstTrimester_VisualCortex FirstTrimester VisualCortex
...
# setting up #####
    ...
    h5ad_path = "23f48c19-6c88-431c-b518-8d8be7ac8706.h5ad" #change here per data
    gene_names_org = "symbol" #change here per data

    # create dev_age dict
    dev_age_dict = {"FirstTrimester": "First_trimester",
    ...
    # create tissue dict
    tissue_dict = {"Neocortex": "neocortex",
                   "Telencephalon": "telencephalon",
                   "Forebrain": "forebrain",
                   "PrefrontalCortex": "prefrontal cortex",
                   "VisualCortex": "visual cortex",
                   "BrodmannArea10": "Brodmann (1909) area 10",
                   "BrodmannArea17": "Brodmann (1909) area 17",
                   "BrodmannArea9": "Brodmann (1909) area 9"}
    ...
    # since the adata is for all of the developmental age and tissue together, we will need to separate them
    print(f"Subsetting the adata for dev_age {dev_age} and {tissue}")
    ...
    cell_id = metadata[(metadata["Group"] == dev_age_dict[dev_age]) & (metadata["tissue"] == tissue_dict[tissue])]["cell_id"] #change here
    ...
    adata_subset.obs.rename(columns={"Class": "cell_type_level_1"}, inplace = True)
    adata_subset.obs.rename(columns={"Subclass": "cell_type_level_2"}, inplace = True)
    adata_subset.obs.rename(columns={"cell_type": "cell_type_level_3"}, inplace = True)
    ...
    # save
    adata_subset.write_h5ad(filename=clean_adata_dev_age_fp)
    ...
    level3_cts = adata_subset.obs["cell_type_level_2"].dropna().unique()
    for ct in level3_cts:
        ct_data = adata_subset[adata_subset.obs["cell_type_level_3"] == ct, :].X
        out = ["level_2", ct, str(ct_data.shape[0])]
        print("|".join(out), file=table3)
