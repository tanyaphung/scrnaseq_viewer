# This script processes the scRNAseq data from adult neocortex from Jorstad et al. 2023 10x
...
# usage: python qc_scrna_16_Jorstad_2023_10x.py 318_Jorstad2023_M1_Human_2023_10x

def main():
# setting up #####
    id = sys.argv[1]
    base_dir = "Preprocessing_scRNA/data" #change here if necessary
    h5ad_path = os.path.join(id, "original_" + id + ".h5ad") #change here per data
    ...
    adata.obs.rename(columns={"Class": "cell_type_level_1"}, inplace = True)
    adata.obs.rename(columns={"WithinArea_subclass": "cell_type_level_2"}, inplace = True)
    adata.obs.rename(columns={"WithinArea_cluster": "cell_type_level_3"}, inplace = True)
    ...
    level3_cts = adata.obs["cell_type_level_3"].dropna().unique()
